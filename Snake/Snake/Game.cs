...
namespace Snake
{
    internal class Game
    {
        ...
        public Game(ref Timer timer, ref Label label, ref Panel panel)
        {
            ...
            Frame = timer;
            LbPontuacao = label;
            offScreenBitmap = new Bitmap(448, 327);
            Snake = new snake();
            food = new food();
            Direction = Keys.Left;
            Arrow = Direction;
        ...
        public void Tick()
        {
            ...
            for (int i = 0; i < Snake.length; i++)
            {
                ...
                {
                    bitmapGraph.FillEllipse(new SolidBrush(ColorTranslator.FromHtml("#4F4F4")), (Snake.Location[i].X * 15), (Snake.Location[i].Y * 15), 15, 15);
                }
